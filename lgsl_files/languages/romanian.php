<?php

//------------------------------------------------------------------------------------------------------------+

  global $lgsl_config;

  $lgsl_config['text']['ttl'] = "Listă Live Servere";
  $lgsl_config['text']['vsd'] = "CLICK AICI PENTRU A VEDEA DETALIILE SERVERULUI";
  $lgsl_config['text']['slk'] = "LINK JOC";
  $lgsl_config['text']['sts'] = "Status";
  $lgsl_config['text']['adr'] = "Adresă";
  $lgsl_config['text']['cpt'] = "Port Server";
  $lgsl_config['text']['qpt'] = "Port Query";
  $lgsl_config['text']['typ'] = "Tip";
  $lgsl_config['text']['gme'] = "Joc";
  $lgsl_config['text']['map'] = "Hartă";
  $lgsl_config['text']['plr'] = "Jucători";
  $lgsl_config['text']['dtl'] = "Detalii";
  $lgsl_config['text']['npi'] = "NU EXISTĂ DATE JUCĂTORI MOMENTAN";
  $lgsl_config['text']['nei'] = "NU EXISTĂ DATE SUPLIMENTARE MOMENTAN";
  $lgsl_config['text']['ehs'] = "Setare";
  $lgsl_config['text']['ehv'] = "Valoare";
  $lgsl_config['text']['onl'] = "ONLINE";
  $lgsl_config['text']['onp'] = "ONLINE (CU PAROLĂ)";
  $lgsl_config['text']['nrs'] = "SERVERUL NU RĂSPUNDE";
  $lgsl_config['text']['pen'] = "ÎN ASTEPTARE INTEROGARE";
  $lgsl_config['text']['zpl'] = "JUCĂTORI:";
  $lgsl_config['text']['mid'] = "ID SERVER INVALID";
  $lgsl_config['text']['nnm'] = "--";
  $lgsl_config['text']['nmp'] = "--";
  $lgsl_config['text']['tns'] = "Servere";
  $lgsl_config['text']['tnp'] = "Jucători";
  $lgsl_config['text']['tmp'] = "Limită jucători";
  $lgsl_config['text']['asd'] = "MOMENTAN ADĂUGAREA PUBLICĂ DE SERVERE ÎN LISTĂ ESTE DEZACTIVATĂ";
  $lgsl_config['text']['awm'] = "ACEASTĂ ZONĂ ÎȚI PERMITE SĂ TESTEZI UN SERVER, APOI SĂ ÎL ADAUGI ÎN LISTA DE SERVERE";
  $lgsl_config['text']['ats'] = "Testare Server";
  $lgsl_config['text']['aaa'] = "SERVERUL ESTE DEJA ADĂUGAT! ESTE NECESARĂ APROBAREA UNUI ADMINISTRATOR";
  $lgsl_config['text']['aan'] = "SERVERUL ESTE DEJA ADĂUGAT";
  $lgsl_config['text']['anr'] = "SERVERUL NU RASPUNDE! ASIGURĂ-TE CĂ AI INTRODUS DATELE CORECTE";
  $lgsl_config['text']['ada'] = "SERVERUL A FOST ADĂUGAT ȘI ASTEAPTĂ APROBAREA UNUI ADMINISTRAOTR";
  $lgsl_config['text']['adn'] = "SERVERUL A FOST ADĂUGAT";
  $lgsl_config['text']['asc'] = "ADĂUGAT CU SUCCES! VERIFICĂ DACA TOTUL ESTE CORECT";
  $lgsl_config['text']['aas'] = "Adăugare Server";
  $lgsl_config['text']['loc'] = "Locație:";
  $lgsl_config['text']['cts'] = "Click pentru a afișa imaginile serverului";
  $lgsl_config['text']['ctb'] = "Click pentru a afișa setările serverului";
  $lgsl_config['text']['lst'] = "Ultima actualizare";
  $lgsl_config['text']['nam'] = "Nume";
  $lgsl_config['text']['gam'] = "Joc";
  $lgsl_config['text']['sta'] = "Status";
  $lgsl_config['text']['sco'] = "Scor";
  $lgsl_config['text']['tim'] = "Timp";
  $lgsl_config['text']['tea'] = "Echipa";
  $lgsl_config['text']['cou'] = "Țara";
  $lgsl_config['text']['pin'] = "Ping";
  $lgsl_config['text']['dea'] = "Decese";
  $lgsl_config['text']['hea'] = "Viața";
  $lgsl_config['text']['ski'] = "Skill";
  $lgsl_config['text']['spe'] = "Spectator";
  $lgsl_config['text']['pag'] = "Pagina";
  $lgsl_config['text']['bak'] = "ÎNAPOI LA LISTA CU SERVERE";
  $lgsl_config['text']['mpg'] = "PAGINA PRINCIPALĂ";
  
  /* admin section */
  $lgsl_config['text']['tzn'] = "m/d/Y H:i:s";
  $lgsl_config['text']['umn'] = "UTILIZATOR";
  $lgsl_config['text']['pwd'] = "PAROLA";
  $lgsl_config['text']['lgn'] = "Conectare";
  $lgsl_config['text']['skc'] = "Salvare (Păstreaza Cache)";
  $lgsl_config['text']['srh'] = "Salvare (Resetare Cache)";
  $lgsl_config['text']['mip'] = "Locații Imagini Hărți";
  $lgsl_config['text']['avm'] = "Manager Avansat";
  $lgsl_config['text']['nrm'] = "Manager Normal";
  $lgsl_config['text']['upd'] = "Actualizări";
  $lgsl_config['text']['yes'] = "DA";
  $lgsl_config['text']['nno'] = "NU";
  $lgsl_config['text']['enb'] = "Activ";
  $lgsl_config['text']['dsb'] = "Inactiv";
  $lgsl_config['text']['aum'] = "UTILIZATORUL ȘI/SAU PAROLA ADMINISTRATORULUI LIPSESC DIN FIȘIERUL DE CONFIGURAȚIE";
  $lgsl_config['text']['apc'] = "PAROLA ADMINISTRATORULUI TREBUIE SĂ FIE DIFERITA DE PAROLA IMPLICITA";
  $lgsl_config['text']['faq'] = "- Pentru a șterge un server din listă trebuie să lași câmpul pentru adresa IP gol și să apeși pe salvare.<br /><br />- Lasă portul pentru query gol pentru a permite LGSL să decidă pentru tine.<br /><br />- Portul software este necesar pentru câteva jocuri, implicit fiind 0.<br /><br />- Editează fișierul lgsl_config.php pentru a activa mai multe optiuni.<br /><br />- Menține LGSL actualizat la zi.";
  $lgsl_config['text']['fso'] = "Necesar pentru interogarea directă a serverelor";
  $lgsl_config['text']['crl'] = "Necesar pentru interogarea serverelor când fsockopen nu este disponibil";
  $lgsl_config['text']['mbs'] = "Necesar pentru a afișa corect numele serverelor sau ale jucătorilor care folosesc caractere UTF-8";
  $lgsl_config['text']['bz2'] = "Necesar pentru a afișa setarile unui server Source când se depășeste limita permisă";
  $lgsl_config['text']['gd2'] = "Necesar pentru a genera imagini ale serverelor";
  $lgsl_config['text']['zli'] = "Necesar pentru jocul America's Army 3";

//------------------------------------------------------------------------------------------------------------+
